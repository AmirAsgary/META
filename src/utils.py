...
def detect_chain_breaks(CA: np.ndarray, chain_idx: np.ndarray, thresh: float = 4.5) -> np.ndarray:
    L = len(CA); brk = np.zeros(L, dtype=bool); brk[0] = True
    if L < 2: return brk
...
def compute_dihedrals(N, CA, C, chain_idx=None, thresh=4.5):
    ...
    L = len(CA); phi, psi, omega = np.full(L, np.nan), np.full(L, np.nan), np.full(L, np.nan)
    if L < 2: return phi, psi, omega
    if chain_idx is None: chain_idx = np.zeros(L, dtype=np.int64)
    vp = ~detect_chain_breaks(CA, chain_idx, thresh)[1:]
...
def compute_bend_cosine(coords, bend_i, bend_j, bend_k):
    v1 = coords[bend_i] - coords[bend_j]; v2 = coords[bend_k] - coords[bend_j]
...
def compute_dihedral_4point(coords, idx_i, idx_j, idx_k, idx_l):
    ...
    n2 = n2 / (np.linalg.norm(n2, axis=-1, keepdims=True) + 1e-8)
    m1 = np.cross(n1, b1 / (np.linalg.norm(b1, axis=-1, keepdims=True) + 1e-8))
    cos_d = np.clip(np.sum(n1 * n2, -1), -1, 1); sin_d = np.clip(np.sum(m1 * n2, -1), -1, 1)
    return sin_d.astype(np.float32), cos_d.astype(np.float32)
# ── Covalent bond encoding ──
def compute_covalent_onehot(edge_src, edge_dst, chain_idx, res_numbers):
...
# ── Triangle properties (bend biochemistry, 3 residues) ──
def compute_triangle_properties(seq_idx, triangles):
    if len(triangles) == 0: return np.zeros((0, 4), dtype=np.float32)
    safe = np.clip(seq_idx, 0, NUM_AA - 1); aa = [AA_ORDER[i] for i in safe]
...
# ── Torsion properties (torsion biochemistry, 4 residues) ──
