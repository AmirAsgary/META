...
# ══════════════════════════════════════════════════════════════════════════════
# Sparse AR Decoder Layer (ProteinMPNN DecLayer pattern)
# ══════════════════════════════════════════════════════════════════════════════
class SparseARDecoderLayer(nn.Module):
    """Single decoder layer: 3-layer MLP over [target, edge, neighbor] messages,
    scatter-add aggregation, residual + LN + FFN. Matches ProteinMPNN DecLayer."""
    def __init__(self, dm, dropout=0.1, scale=30):
        super().__init__(); self.scale = scale
        # MLP: 4*dm -> dm -> dm -> dm (input = [h_V_target, h_E, h_S_nbr, h_V_nbr])
        self.W1 = nn.Linear(dm * 4, dm, bias=True)
        self.W2 = nn.Linear(dm, dm, bias=True)
        self.W3 = nn.Linear(dm, dm, bias=True)
        self.act = nn.GELU()
        self.norm1 = nn.LayerNorm(dm); self.norm2 = nn.LayerNorm(dm)
        self.dropout1 = nn.Dropout(dropout); self.dropout2 = nn.Dropout(dropout)
        self.dense = nn.Sequential(nn.Linear(dm, dm * 4), nn.GELU(), nn.Dropout(dropout), nn.Linear(dm * 4, dm))
    def forward(self, h_V, h_ESV_mixed, edge_dst, N):
        """h_V: (N,dm) node states, h_ESV_mixed: (E,3*dm) causal messages, edge_dst: (E,) targets."""
        h_target = h_V[edge_dst]  # (E, dm)
        h_msg_in = torch.cat([h_target, h_ESV_mixed], -1)  # (E, 4*dm)
        h_msg = self.W3(self.act(self.W2(self.act(self.W1(h_msg_in)))))  # (E, dm)
        # scatter-add to destination nodes, fixed normalization (like ProteinMPNN /scale)
        dh = torch.zeros(N, h_msg.shape[-1], device=h_msg.device, dtype=h_msg.dtype)
        dh.scatter_add_(0, edge_dst.unsqueeze(-1).expand_as(h_msg), h_msg)
        dh = dh / self.scale
        h_V = self.norm1(h_V + self.dropout1(dh))
        h_V = self.norm2(h_V + self.dropout2(self.dense(h_V)))
        return h_V
# Full Sparse AR Decoder
class SparseARDecoder(nn.Module):
    """ProteinMPNN-style AR decoder: sparse graph causal message masking.
    Per-edge binary mask splits neighbors into 'past' (seq+structure) and
    'future' (structure-only). Parallel training, sequential generation.
    Conditioned on predicted MSF, bend context, torsion context."""
    def __init__(self, dm, n_aa=20, n_msf_bins=32, n_layers=2, dropout=0.1, scale=30):
        ...
        self.cond_proj = nn.Linear(dm + dm // 4 + dm + dm, dm)  # h0 || msf || bend || torsion
        self.decoder_layers = nn.ModuleList([SparseARDecoderLayer(dm, dropout, scale) for _ in range(n_layers)])
        self.head = nn.Linear(dm, n_aa)
        # xavier init
        for p in self.parameters():
            if p.dim() > 1: nn.init.xavier_uniform_(p)
    def _build_causal_mask(self, perm, edge_src, edge_dst, node_batch=None):
        """Per-edge binary causal mask: 1 if neighbor (src) decoded before target (dst).
        Returns (E, 1) float tensor. Cross-protein edges blocked if node_batch given."""
        N = perm.shape[0]; dev = perm.device
        order = torch.zeros(N, dtype=torch.long, device=dev)
        order[perm] = torch.arange(N, device=dev)
        mask_bw = (order[edge_src] < order[edge_dst]).float().unsqueeze(-1)  # (E, 1)
        if node_batch is not None:
            same = (node_batch[edge_src] == node_batch[edge_dst]).float().unsqueeze(-1)
            mask_bw = mask_bw * same
        return mask_bw
    def _build_messages(self, h_V, h_V_enc, h_E_enc, h_S, edge_src, mask_bw):
        """Build per-edge mixed messages. Past: [edge, seq_nbr, node_nbr(updated)].
        Future: [edge, zeros, node_nbr(frozen encoder)]. Returns (E, 3*dm)."""
        mask_fw = 1.0 - mask_bw
        h_past = torch.cat([h_E_enc, h_S[edge_src], h_V[edge_src]], -1)
        h_future = torch.cat([h_E_enc, torch.zeros_like(h_S[edge_src]), h_V_enc[edge_src]], -1)
        return mask_bw * h_past + mask_fw * h_future
    def _decode_pass(self, cond, h_V_enc, h_E_enc, h_S, edge_src, edge_dst, mask_bw, N):
        """Single decoder forward pass: init from cond, aggregate causal messages."""
        h_V = cond.clone()
        for layer in self.decoder_layers:
            h_ESV = self._build_messages(h_V, h_V_enc, h_E_enc, h_S, edge_src, mask_bw)
            h_V = layer(h_V, h_ESV, edge_dst, N)
        return self.head(h_V)
    def forward(self, h_V_enc, h_E_enc, edge_src, edge_dst, msf_bins, bend_ctx, torsion_ctx,
                seq_gt, perm, node_batch=None, sched_sample_ratio=0.0):
        """Parallel teacher-forced AR decoding with graph-sparse causal masking.
        h_V_enc: (N,dm) encoder node latents, h_E_enc: (E,dm) encoder edge latents,
        edge_src/dst: (E,) contact graph edges, seq_gt: (N,) GT AA indices,
        perm: (N,) decoding permutation, sched_sample_ratio: fraction for scheduled sampling."""
        N, dev = h_V_enc.shape[0], h_V_enc.device
        cond = self.cond_proj(torch.cat([h_V_enc, self.msf_emb(msf_bins), bend_ctx, torsion_ctx], -1))
        mask_bw = self._build_causal_mask(perm, edge_src, edge_dst, node_batch)
        # scheduled sampling (Phase 4): two-pass — first pass gets causal predictions,
        # then 20% of GT tokens are replaced with model predictions, second pass computes loss
        if sched_sample_ratio > 0 and self.training:
            with torch.no_grad():
                h_S_gt = self.aa_emb(seq_gt.clamp(0, self.n_aa - 1))
                pred_logits = self._decode_pass(cond, h_V_enc, h_E_enc, h_S_gt, edge_src, edge_dst, mask_bw, N)
        ...
        h_S = self.aa_emb(seq_input)
        return self._decode_pass(cond, h_V_enc, h_E_enc, h_S, edge_src, edge_dst, mask_bw, N)
    @torch.no_grad()
    def generate(self, h_V_enc, h_E_enc, edge_src, edge_dst, msf_bins, bend_ctx, torsion_ctx,
                 perm, temp=1.0, top_p=0.9):
        """True autoregressive generation: decode one position at a time."""
        N, dev = h_V_enc.shape[0], h_V_enc.device
        cond = self.cond_proj(torch.cat([h_V_enc, self.msf_emb(msf_bins), bend_ctx, torsion_ctx], -1))
        S = torch.full((N,), self.n_aa, dtype=torch.long, device=dev)
        ...
        for t in range(N):
            ...
            # at step t, target is perm[t], order[perm[t]]=t
            # a src is "past" if order[src] < order[target] = t, so decoded_mask = (order[src] < t)
            decoded_mask = (order[edge_src] < t).float().unsqueeze(-1)
            h_S = self.aa_emb(S.clamp(0, self.n_aa - 1))
            h_V = cond.clone()
            for layer in self.decoder_layers:
                h_ESV = self._build_messages(h_V, h_V_enc, h_E_enc, h_S, edge_src, decoded_mask)
                h_V = layer(h_V, h_ESV, edge_dst, N)
            logits_i = self.head(h_V)[i] / temp
            probs = F.softmax(logits_i, -1)
            # nucleus (top-p) sampling
            sp, si = probs.sort(descending=True)
            cumsum = sp.cumsum(-1) - sp; mk = cumsum > top_p
            sp[mk] = 0; sp = sp / (sp.sum() + 1e-12)
            chosen = si[torch.multinomial(sp, 1).squeeze()]
            S[i] = chosen; log_probs[i] = torch.log(probs[chosen] + 1e-12)
        return S, log_probs
# ══════════════════════════════════════════════════════════════════════════════
# parse_layer_types
# ══════════════════════════════════════════════════════════════════════════════
...
class METAModel(nn.Module):
    ...
    def forward(self, batch, sched_sample_ratio=0.0):
        ...
        out = {}
        # primary predictions
        # FIX #6: skip one-shot seq_decoder when AR active during training (save compute)
        ...
        if self.use_ar and hasattr(self, 'ar_decoder'):
            ...
            out['ar_logits'] = self.ar_decoder(h[0], h[1], batch['edge_src'], batch['edge_dst'], msf_bins, bend_ctx, torsion_ctx, batch['seq_idx'], perm, nb, sched_sample_ratio)
